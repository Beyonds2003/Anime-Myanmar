...
function Header() {
  ...
  const [changeLan, setChangeLan] = React.useState(
    cookies.get("googtrans") || "en-Us"
  );
  const [search, setSearch] = React.useState("");
  const [searchHistory, setSearchHistory] = React.useState(
    JSON.parse(localStorage.getItem("AmSh")) || []
  );
  const [watchHistory, setWatchHistory] = React.useState(false);
  const [history, setHistory] = React.useState(
    JSON.parse(localStorage.getItem("AmTime"))
  );
  const [amac, setAmac] = React.useState(
  ...
  return (
    <div className="main">
      <div className="name_container">
        <div onClick={() => navigate("/")} className="titleHeader">
          <img
            src="./anime_myanmar.png"
        ...
        <div className="flex headerIconMain">
                      ...
                      <div>
                        {searchHistory.map((data, index) => (
                          <Typography
                            ...
                            key={index}
                            onClick={(e) => (
                              navigate(`/v1/search?query=${data}`),
                              setCheckS((pres) => !pres),
                              setOpenSearchBox(false),
                              setSearchHistory((pres) =>
                                searchHistory[0] === data
                                  ? searchHistory
                                  : [
                                      data,
                                      ...pres.filter((info) => info != data),
                                    ]
                              )
                            )}
                          >
                            <span style={{ color: "#0395F5" }}>
                              {index + 1}.
                            </span>{" "}
                            {data}
                          </Typography>
              ...
              {search.length > 0 && openSearchBox === true && (
                <div className="lan2">
                  {anime
                    .filter((dada) => {
                      if (
                        dada.title.toLowerCase().includes(search.toLowerCase())
                      ) {
                        return dada;
                      }
                    })
                    .map((info, index) => (
                      <Typography
                        ...
                        className="historyList"
                        onClick={(e) => (
                          navigate(`/v1/search?query=${info.title}`),
                          setOpenSearchBox(false),
                          setCheckS((pres) => !pres),
                          setSearchHistory((pres) =>
                            searchHistory[0] === info.title
                              ? searchHistory
                              : [
                                  info.title,
                                  ...pres.filter(
                                    (infos) => infos != info.title
                                  ),
                                ]
                          ),
                          setSearch("")
          ...
          <div className="icons">
            ...
            {language && (
              <div
                ...
                onMouseLeave={() => setLanguage(false)}
              >
                <div className="arrowup" onMouseEnter={() => setLanguage(true)}>
                  <ArrowDropUpIcon style={{ fontSize: "40px" }} />
                </div>
                <Typography
                  variant="body2"
          ...
          {amac.length === 0 || Pload === false ? (
            <div className="icons">
              <Button id="but" onClick={() => setSign(true)}>
                Sign In
              </Button>
            </div>
          ) : (
            <div
              className="avatarConc"
              ...
              {profile && (
                ...
                >
                  ...
                  >
                    <div
                      className="flexNavPc"
                      onClick={() => navigate("/v1/myAccount")}
                    >
                      <PermIdentityIcon />
                      <Typography id="navText" className="notranslate">
                        My Account
                      </Typography>
                    </div>
                    <div>
