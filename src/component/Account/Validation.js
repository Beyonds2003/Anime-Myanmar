...
function Validation({
  ...
  const handleVerify = async () => {
    try {
      if (!conditionOfValidPage) {
        const { data } = await axios.post(`${url}v1/signUp/verify`, {
          userId,
          otp,
        });
        ...
        const { data } = await axios.post(`${url}v1/recoverPassword/verify`, {
          userId,
          otp,
        });
  ...
  const handleResendVerification = async () => {
    try {
      const { data } = await axios.post(`${url}v1/signUp/resendVerify`, {
        userId,
        emailFromS,
      });
      ...
      <div style={{ textAlign: "center", padding: "0px 30px" }}>
        ...
        {num > 0 ? (
          <Typography style={{ marginTop: "30px" }}>
            Wait <span className="blueS">{`${num}s`}</span> before requesting
          ...
          >
            <span className="blueS">Re-send verification code</span>
          </Typography>
        )}
        <Typography id="junk">
          If you have not received the verification code, try checking your junk
          folder.
        </Typography>
      </div>
    </div>
