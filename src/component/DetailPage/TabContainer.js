...
function TabContainer({ firstEpNum, animecount, params, currentData }) {
  ...
  React.useEffect(() => {
    const dynamicList = () => {
      let divs = [];
      for (let i = 0; i < animecount; i = i + 25) {
        let num = 1 + i;
        let num1 = num + 24;
        if (num1 > animecount) {
          divs.push(
            <button
              ...
              value={num}
              onClick={handleClick}
              className="butforV  notranslate"
            >
              {`${num}`} {num !== animecount && -animecount}
            </button>
          );
        } else {
          divs.push(
  ...
  React.useEffect(() => {
    const fetchEpisodes = async () => {
      try {
        setCheck2(false);
        const { data } = await axios.post(`${url}v1/episodes/${params.name}`, {
          skipAmount: nums - 1,
        });

        setEpData(data.episodes);
        setCheck2(true);
      } catch (error) {
        console.log(error);
        setCheck2(false);
    ...
    <div className="tapContainer">
      <Box sx={{ width: "100%", typography: "body1" }}>
        <TabContext value={value}>
          <Box
            sx={{ borderBottom: 1, borderColor: "rgba(255 243 243 / 0.3);" }}
          >
          ...
          <TabPanel value="1" style={{ padding: "0px" }}>
            {animecount > 1 && (
              <div
                className="eptab"
                onClick={() => setDropBox((pres) => !pres)}
              >
                <div className="tab-child">
                  <span style={{ display: "flex", height: "20px" }}>
                    Episodes &nbsp;{" "}
                    <div className="notranslate" style={{ width: "80px" }}>
                      {storeList[Math.floor(nums / 25)]}
                    </div>
                  </span>
                  <span>
                    <ExpandMoreIcon />
            ...
            {dropbox && (
              <div className="list">
                {storeList.map((data) => (
                  <div className="child_list" key={data.key}>
                    {data}
                  </div>
                ))}
              </div>
              ...
              {check2 && (
                <Grid container direction="row">
                  {epData.map((vid, index) => (
                    ...
                    >
                      <VideoComponent num={index} vid={vid} />
                    </Grid>
                  ))}
                </Grid>
              )}
