...
function Recommend({ info, setOpenAlert, setOpenAlert2 }) {
  ...
  const { userInfo, setSign, url, setUserInfo } =
    ...
    >
      <div className="remCon" onClick={handleGoRecommend}>
        <div className="recommendImg">
          ...
          <div className={hover ? "absoRem" : ""} onClick={handleBlack}>
            <div style={{ display: hover ? "block" : "none" }}>
              <PlayCircleIcon
                ...
                id="playbutRem"
                style={{ color: playHover ? "#2f9ddd" : "#0486D2" }}
              />
            </div>
            <div style={{ display: hover ? "block" : "none" }}>
              <div
                className="sharebutRem"
          ...
          {userInfo ? (
            <>
              <Typography
                id="watRem"
                style={{ display: saveHover ? "block" : "none" }}
              >
                {userInfo.watchLater.includes(info.title)
        ...
        <Typography
          ...
          id="remTit"
          style={{ color: hover ? "#009eff" : "white" }}
        >
          {info?.title}
        </Typography>
        <Typography
          ...
          style={{
            marginTop: "10px",
            marginLeft: "5px",
            color: hover ? "#009eff" : "white",
          }}
          gutterBottom
        >{`${info?.title.split("").slice(0, 12).join("")}${
          info?.title.split("").length > 12 ? "..." : ""
        }`}</Typography>
      </div>
    </motion.div>
  );
