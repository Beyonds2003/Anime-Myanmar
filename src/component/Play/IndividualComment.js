...
function IndividualComment({ coms, setComm, comm, pTitle, pEpname, socket }) {
  const { userInfo, setSign, url, commentOpenBox, setCommentOpenBox } =
    React.useContext(AnimeContext);

  const [likeCom, setLikeCom] = React.useState(
    userInfo ? coms.like.includes(userInfo.id) : false
  );
  const [likeCount, setLikeCount] = React.useState(coms.like.length);
  const [dislikeCom, setDislikeCom] = React.useState(
    userInfo ? coms.dislike.includes(userInfo.id) : false
  );
  const [likeArray, setLikeArray] = React.useState(
    userInfo ? coms.like.filter((item) => item === userInfo.id) : []
  );
  const [dislikeArray, setDislikeArray] = React.useState(
    userInfo ? coms.dislike.filter((item) => item === userInfo.id) : []
  );
    ...
    if (userInfo) {
      ...
      if (likeArray.includes(userInfo.id)) {
        ...
        const likes = await axios.patch(
          `${url}v1/play/${pTitle}/${pEpname}/comments/like`,
          { commentId: coms._id, userId: userInfo.id }
        );

        setLikeCount((pres) => pres - 1);
      } else {
        const filterDisLike = dislikeArray.filter(
          (info) => info !== userInfo.id
        );
        ...
        const dislike = await axios.patch(
          `${url}v1/play/${pTitle}/${pEpname}/comments/like`,
          { commentId: coms._id, userId: userInfo.id }
        );
  ...
  const handleDislikeCom = async () => {
    if (userInfo) {
      ...
      if (dislikeArray.includes(userInfo.id)) {
        ...
        const dislike = await axios.patch(
          `${url}v1/play/${pTitle}/${pEpname}/comments/dislike`,
          { commentId: coms._id, userId: userInfo.id }
        );
      } else {
        const filterLike = likeArray.filter((info) => info !== userInfo.id);
    ...
    const fetchReply = async () => {
      try {
        ...
        const { data } = await axios.post(
          `${url}v1/play/${pTitle}/${pEpname}/replys`,
          { skip: skipReply, reply: true, parentId: coms._id }
        );
  ...
  const handleAddReply = async (e) => {
    if (userInfo) {
      const jojo = await axios.post(
        `${url}v1/play/${pTitle}/${pEpname}/createComment`,
        {
          userId: userInfo.id,
          name: userInfo.name,
          parentId: coms._id,
  ...
  return (
    <div className="allCommContainer">
      <div className="flex">
        <div className="avatar3 notranslate">
          <Avatar
            style={{ width: "45px", height: "45px", fontSize: "19px" }}
            {...stringAvatar(`${coms.name}`)}
          />
        </div>
        <div style={{ display: "block", width: "100%" }}>
          <div style={{ display: "flex", alignItems: "center" }}>
            <Typography id="comUserName">{coms.name}</Typography>
            <Typography id="comDate">{`${formatDistance(
              new Date(),
              Date.parse(coms.time)
            )} ago`}</Typography>
          </div>
          ...
          <div className="flex flexAlignCom">
            <div style={{ display: "flex", alignItems: "center" }}>
              {!likeCom ? (
                <IconButton onClick={handleLikeCom} style={{ padding: "0px" }}>
                  <ThumbUpOffAltIcon id="comThumbup" />
                </IconButton>
              ) : (
                <IconButton onClick={handleLikeCom} style={{ padding: "0px" }}>
                  <ThumbUpIcon id="comThumbup" />
                </IconButton>
              )}
              <Typography
                style={{
            ...
            >
              {!dislikeCom ? (
                <IconButton
                  onClick={handleDislikeCom}
                  style={{ padding: "0px" }}
                >
                  <ThumbDownOffAltIcon id="comThumbup" />
                </IconButton>
              ) : (
                <IconButton
                  onClick={handleDislikeCom}
          ...
          {replyBox && (
            <div className="addReplyKid">
              <div className="avatar9 notranslate ">
                <Avatar
              ...
              <div className="replyBox">
                ...
                <div className="comCon">
                  <Button
                    onClick={() => (setReplyBox(false), setReplyCom(""))}
                    id="cancelComment"
                  >
                    Cancel
                  </Button>
                  <Button
                    disabled={replyCom.length > 0 ? false : true}
          ...
          {fetchReply && (
            <div>
              {ownCom.length > 0 && (
                <div
                  className="viewMoreReply"
                  style={{ marginBottom: showReply ? "0px" : "-15px" }}
                  onClick={() => setShowReply((pres) => !pres)}
                >
                  {!showReply ? (
                    <>
                      ...
                      >{`View ${replyCount} replies`}</Typography>
                    </>
                  ) : (
                    <>
                      ...
                      <Typography
                        style={{ fontSize: "15px", fontWeight: "600" }}
                      >{`Hide ${replyCount} replies`}</Typography>
                    </>
                  )}
                </div>
              )}
            ...
            <div>
              {ownCom.length > 0 && (
                <div
                  style={{
                    userSelect: "none",
                    marginBottom: showReply ? "-23px" : "5px",
                  }}
                >
                  {showReply &&
                    ownCom.map((replys, index) => (
                      <ReplyComment
                        key={replys._id}
                        setOwnCom={setOwnCom}
                        pTitle={pTitle}
