...
function CommentPage({
  ...
  const { userInfo, url, setSign, commentOpenBox } =
    ...
    const fetchComments = async () => {
      try {
        ...
        const { data } = await axios.post(
          `${url}v1/play/${pTitle}/${pEpname}/comments`,
          { skip: skips, reply: false }
        );
        ...
        setSkips(0);
        const scollDiv = document.getElementsByClassName(
          "scrollEventControllerDiv"
        )[0];
  ...
  return (
    <div
      className="scrollEventControllerDiv"
      style={{
        height:
          window.innerWidth > 764
            ? "400px"
            : "calc( 100vh - ( 60vw + 138px ) )",
      }}
      ...
      {userInfo && (
        <div className="addCommentKid">
          <div className="avatar3 notranslate">
            <Avatar id="comAvatar" {...stringAvatar(`${userInfo.name}`)} />
          </div>
          <div className="commentBox">
            ...
            {click ? (
              <div className="comCon">
                <Button
                  onClick={() => (setClick(false), setComment(""))}
                  id="cancelComment"
                >
                  Cancel
                </Button>
      ...
      {fetchCom ? (
        <div className="individualCommentContainer">
          {!errorNoComment ? (
            <div>
              {socketComm
                .filter((item) => socketComm.indexOf(item._id) !== item._id)
                .map((coms, index) => (
                  <IndividualComment
              ...
              <div>
                {!fetchMore &&
                socketComm.length < commentCounts &&
                handleScrollEvent ? (
                  <div
