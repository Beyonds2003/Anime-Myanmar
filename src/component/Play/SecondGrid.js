...
function SecondGrid({ ep, load, setPepName, pTitle, pEpname, epC }) {
  ...
  const handleRequest = (e) => {
    let lastRoute;
    if (epC > 1) {
      lastRoute = `${pTitle} Episode ${e.target.value}`;
      setPepName(`${pTitle} Episode ${e.target.value}`);
    } else {
      lastRoute = `${pTitle} ${e.target.value}`;
      setPepName(`${pTitle} ${e.target.value}`);
    }
    navigate(`/v1/play/${pTitle}/${lastRoute}`);
  };
      ...
      if (num > counts) {
        divs.push(
          <TabPanel
            ...
            style={{ padding: "0" }}
          >
            {<Grid container>{List(i + 1, counts)}</Grid>}
          </TabPanel>
        );
      } else if (i < counts) {
        divs.push(
          <TabPanel
            ...
            style={{ padding: "0" }}
          >
            {<Grid container>{List(i + 1, num)}</Grid>}
          </TabPanel>
        );
      }
    }
    ...
    if (load) {
      ...
      for (let i = no; i <= num; i++) {
        if (i === Number(currentEp)) {
          divs.push(
            <Grid item key={i}>
              <Paper elevation={6} id="card_con">
                <Typography className="notranslate" id="card">
                  <button
                    ref={currentRef}
                    className="savior notranslate"
                    value={i}
  ...
  return (
    <Grid
      ...
      order={{ xs: 3, sm: 3, md: 3, lg: 2, xl: 2 }}
    >
      {load && (
        <div>
          <div>
          ...
          <div style={{ position: "relative" }}>
            <div className="epConc">
              ...
              <Typography style={{ color: "#009eff", zIndex: "1" }}>
                Episodes
              </Typography>
            </div>
            <div className="black">
