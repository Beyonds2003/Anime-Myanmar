...
function Play() {
  ...
  const [acc, setAcc] = React.useState(
    JSON.parse(localStorage.getItem("AmAc")) || []
  );
  ...
  React.useEffect(() => {
    const fetchEpisode = async () => {
      ...
      try {
        ...
        setEp(data);
        setLoad(true);
        setLikeCount(data.getDataInsideOneEpisode[0].episodes[0].likes.length);
        setCommentCounts(data.commentCount);
        if (userInfo) {
          setPlayLike(
            data.getDataInsideOneEpisode[0].episodes[0].likes.filter(
              (item) => item === userInfo.id
            )
          );
          setLike(
            data.getDataInsideOneEpisode[0].episodes[0].likes.includes(
              userInfo.id
            )
          );
        } else {
  ...
  React.useEffect(() => {
    if (userInfo && !fuck && load) {
      setPlayLike(
        ep.getDataInsideOneEpisode[0].episodes[0].likes.filter(
          (item) => item === userInfo.id
        )
      );
      setFuck(true);
      ...
      if (userInfo) {
        setLike(
          ep.getDataInsideOneEpisode[0].episodes[0].likes.includes(userInfo.id)
        );
      } else {
        setLike(false);
      }
  ...
  const handleAddWatchLaterButton = async () => {
    if (acc.length < 1) {
      ...
      try {
        const dada = await axios.patch(
          `${url}v1/addWatchLater/${userInfo.id}`,
          { animeTitle: ep.getDataInsideOneEpisode[0].title }
        );
          ...
          <Grid container>
            ...
            <Grid ref={stikcyRef} id="setH" item sm={12} md={12} lg={9} xl={9}>
              <div className="playContainer">
                <Video
                  ep={ep.getDataInsideOneEpisode[0].episodes}
                  pEpname={pEpname}
                  isReady={isReady}
              ...
              >
                <div className="flex">
                  <div className="Marg flex" onClick={handleLike}>
                    {!like ? (
                      <ThumbUpOffAltIcon id="thumbOff" />
                    ) : (
                      <ThumbUpIcon id="thumbUp" />
                    )}
                    <Typography id="likePc">
                      {" "}
                      {likeCount}{" "}
                      <span>{likeCount > 0 ? "Likes" : "Like"}</span>
                    </Typography>
                  </div>
                  <div
                    className="Marg flex"
                  ...
                  >
                    <div>
                      <ChatBubbleOutlineIcon id="thumbOff" />
                    </div>
                    <Typography
                      id="fontForV"
                ...
                <div className="flex">
                  {userInfo ? (
                    <>
                      {userInfo.watchLater.includes(
                        ep.getDataInsideOneEpisode[0].title
                      ) ? (
                        <div
                          className="Marg flex"
                          onClick={handleRemoveWatchLaterButton}
                        >
                          <BookmarkAddedIcon id="thumbOff" />
                          <Typography
                            id="fontForV"
                        ...
                        <div
                          className="Marg flex"
                          onClick={handleAddWatchLaterButton}
                        >
                          <BookmarkAddIcon id="thumbOff" />
                          <Typography
                            id="fontForV"
                            style={{ margin: "0px 0px 0px 5px" }}
                          >
                  ...
                  <div onClick={handleShare} className="Marg flex">
                    <IosShareIcon id="thumbOff" />
                    <Typography
                      id="fontForV"
            ...
            <SecondGrid
              ...
              epC={ep.epCount}
              load={load}
              setPepName={setPepName}
              pTitle={pTitle}
              pEpname={pEpname}
            />
            <Description
              ep={ep.getDataInsideOneEpisode[0].episodes[0]}
              dada={ep.getDataInsideOneEpisode[0]}
              epCount={ep.epCount}
              load={load}
            />
          </Grid>
